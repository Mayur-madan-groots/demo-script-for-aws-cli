...
#replacing OLD IP with NEW IP in config file
sed -i'.BACKUP' "s/$POIP/$PIP/g" /var/lib/jenkins/.ssh/config
#sed -i "s/$POIP/$PIP/g" /var/lib/jenkins/.ssh/config
...
#ssh -t  -o StrictHostKeyChecking=no -i /var/lib/jenkins/.ssh/alcemy_tech ubuntu@$PIP ls -lrta
#ssh -o StrictHostKeyChecking=no test ls -lrta
#ssh -o StrictHostKeyChecking=no test 'ls -lrta; sudo chown ubuntu /var/www/html/index.html;'
#ssh -t  -o StrictHostKeyChecking=no -i /var/lib/jenkins/.ssh/alcemy_tech ubuntu@$PIP sudo chown ubuntu /var/www/html/index.html
ssh -o StrictHostKeyChecking=no test sudo chown ubuntu /var/www/html/index.html
#scp -t  -o StrictHostKeyChecking=no -i /var/lib/jenkins/.ssh/alcemy_tech /var/lib/jenkins/index.html/ ubuntu@$PIP:/var/www/html
scp -o StrictHostKeyChecking=no /var/lib/jenkins/index.html test:/var/www/html
#ssh -t  -o StrictHostKeyChecking=no -i /var/lib/jenkins/.ssh/alcemy_tech ubuntu@PIP sudo systemctl restart apache2
